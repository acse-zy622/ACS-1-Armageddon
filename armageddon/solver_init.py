...
class Planet():
    ...
    def solve_atmospheric_entry(
        ...
        A = np.pi*self.radius[-1]**2
        dvdt = -self.Cd*A*rhoa*velocity[-1]**2/(2*self.mass[-1])
        dmdt = -self.Ch*rhoa*A*self.velocity[-1]**3/(2*self.Q)
        dthetadt = ((self.g*cos_theta/velocity[-1]) 
                    - self.Cl*rhoa*A*self.velocity[-1]/(2*self.mass[-1])
    ...
    def cal_tabu_dens():
        """
        Function to calculate density when the input is tabular data
        """
        pass
